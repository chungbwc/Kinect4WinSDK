// Kinect4WinSDK.cpp : Defines the exported functions for the DLL application.
//

#include "stdafx.h"
#include "Kinect4WinSDK.h"
#include <iostream>

using namespace std;

Kinect4WinSDK::Kinect4WinSDK(void)
{
...
DWORD WINAPI Kinect4WinSDK::Nui_ProcessThread(LPVOID pParam)
{
	Kinect4WinSDK* pThis = (Kinect4WinSDK *) pParam;
	return pThis->Nui_ProcessThread();
}

DWORD WINAPI Kinect4WinSDK::Nui_ProcessThread()
{
	const int NUM = 4;
	HANDLE hEvents[NUM] = {
		m_hEvNuiProcessStop,
...
HRESULT Kinect4WinSDK::Nui_Init()
{
	...
	if (FAILED(hr))
	{
		cout << "(Init) get sensor count failed " << hr << endl;
		return hr;
	}
	for (int i=0; i<cnt; i++)
	{
		...
		if (FAILED(hr))
		{
			cout << "(Init) create sensor failed " << hr << endl;
			continue;
		}
	...
	if (m_pNuiSensor == NULL || FAILED(hr)) {
		cout << "(Init) Initialize sensor failed " << hr << endl;
		return hr;
	}
...
void Kinect4WinSDK::Nui_GotDepthAlert()
{
...
void Kinect4WinSDK::Nui_GotSkeletonAlert()
{
...
int * Kinect4WinSDK::Nui_GetImage()
{
	return pixels;
}

int * Kinect4WinSDK::Nui_GetDepth()
{
	return depth;
}

int * Kinect4WinSDK::Nui_GetMask()
{
	return mask;
}
